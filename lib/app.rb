...
Dotenv.load('../.env') # Ceci appelle le fichier .env et grâce à la gem Dotenv, on importe toutes les données enregistrées dans un hash ENV
...
def tweet
    return client.update('Mon premier tweet en Ruby !!!!')                  # ligne qui permet de tweeter sur ton compte
end
...
def tweet_journalist
    ...
    5.times {|num| journalists_five << journalists.sample}

    journalists_five.each {|journalist|
        content_tweet = "#{journalist} je te remercie pour leur rôle central dans le fonctionnement de notre démocratie\n #bonjour_monde"
        client.update(content_tweet)
    }
end

# 2-3. Il va liker bonjour -  Il va follow bonjour
def like_follow
    last_tweet_hello = {}
    client.search("#bonjour_monde", result_type: "recent").take(25).collect do |tweet|
        last_tweet_hello[tweet.id] = tweet.user.screen_name
    end
    last_tweet_hello.each {|k,v|
        client.follow("v")
        client.like(k)
    }
end
